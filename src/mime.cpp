...
Mime::Mime( const std::string &p_user_agent )
...
void Mime::buildAttachments(std::vector<std::string> &p_contents) const
{
    for (const std::string &attachment : m_files)
    {
        ...
        if ( !ifs.good() )
        {
            throw std::runtime_error("[!] Input stream for file: " + attachment + " is not in a good state!");
        }
...
{
    std::string contents;
    for (const std::string &line : build()) contents += line;
